...
if (theme_get_setting('beam_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('beam_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'beam') .'/css/tabs.css', 'theme', 'screen');
}
...
function beam_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('beam_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = beam_id_safe('page-'. $path);
    $body_classes[] = beam_id_safe('section-'. $section);
...
function beam_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = beam_id_safe('node-type-' . $vars['type']);
...
function beam_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = beam_id_safe('block-' . $vars['block']->module);
    $classes[] = beam_id_safe('block-' . $vars['block']->region);
    $classes[] = beam_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('beam_block_editing') && user_access('administer blocks')) {
...
function beam_preprocess_comment(&$vars, $hook) {
...
function beam_menu_item_link($link) {
...
function beam_menu_local_tasks() {
  $output = '';
...
function beam_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = beam_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function beam_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
